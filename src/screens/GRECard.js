...
class GRECard extends Component {
  ...
  searchForWord = async (index) => {
    ...
    logger('current word = ' + gre[index]);
    let [err, data] = await to(DatabaseSearcher.searchDatabase(gre[index], this.props.dbInstance));
  ...
  componentWillMount = async () => {
    ...
    if(this.props.navigation.state.params && this.props.navigation.state.params.index) {
      ...
      WordIndexer.setWordIndex('gre', index);
    }
    else {
      index = await WordIndexer.getWordIndex('gre');
    }
    this.setState({ index });
        ...
        <Container>
          ...
          <Header>
            ...
            <Body style={{ flex: 2, alignItems: 'center' }}>
              ...
              <Text style={{ color: 'white', fontFamily: 'Lato-Bold' }}>{ this.state.index + 1 } / { Constants.greLength }</Text>
            </Body>
            <Right style={{ flex: 1 }}></Right>
          </Header>
