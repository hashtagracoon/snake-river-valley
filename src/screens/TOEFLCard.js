...
class TOEFLCard extends Component {
  ...
  componentWillMount = async () => {
    ...
    if(this.props.navigation.state.params && this.props.navigation.state.params.index) {
      ...
      WordIndexer.setWordIndex('toefl', index);
    }
    else {
      index = await WordIndexer.getWordIndex('toefl');
    }
    this.setState({ index });
        ...
        <Container>
          ...
          <Header>
            ...
            <Body style={{ flex: 2, alignItems: 'center' }}>
              ...
              <Text style={{ color: 'white', fontFamily: 'Lato-Bold' }}>{ this.state.index + 1 } / { Constants.toeflLength }</Text>
            </Body>
            <Right style={{ flex: 1 }}></Right>
          </Header>
